  ...
  async _loadSettings() {
    try {
      const { data } = await supabase.from('settings').select(
        'sellauth_api_key, sellauth_shop_id, sellauth_shop_url, coupon_enabled, coupon_discount, coupon_type, coupon_description, coupon_max_uses, coupon_schedule_hour'
      ).single();
      const result = data || {};
      // ENV-Vars haben immer Vorrang vor DB-Werten
