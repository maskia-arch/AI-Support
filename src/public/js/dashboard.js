...
async function loadSellauthConfigStatus() {
    ...
    try {
        ...
        if (c.ready) {
            statusEl.innerHTML =
                '<span style="color:#22c55e;font-size:0.82rem;">✅ API Key: ' + esc(c.apiKey) + '</span>' +
                '<br><span style="color:#22c55e;font-size:0.82rem;">✅ Shop ID: ' + esc(c.shopId) + '</span>' +
...
async function syncSellauth() {
    ...
    try {
        ...
        showToast('❌ ' + errMsg);
        // Config-Status neu laden um fehlende Variablen anzuzeigen
