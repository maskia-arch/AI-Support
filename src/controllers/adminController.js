...
function mergeEnvIntoSettings(data) {
  ...
  if (process.env.SELLAUTH_API_KEY) {
    result.sellauth_api_key     = null;   // nie zurückgeben
    result.sellauth_api_key_via_env = true;
  }
  if (process.env.SELLAUTH_SHOP_ID) {
    result.sellauth_shop_id     = process.env.SELLAUTH_SHOP_ID;
    result.sellauth_shop_id_via_env = true;
  }
  if (process.env.SELLAUTH_SHOP_URL) {
    result.sellauth_shop_url    = process.env.SELLAUTH_SHOP_URL;
...
const adminController = {
  ...
  async lookupInvoice(req, res, next) {
    try {
      ...
      const { apiKey, shopId, shopUrl } = getEffectiveSellauth(s);
      if (!apiKey) return res.status(400).json({ error: 'Sellauth nicht konfiguriert (SELLAUTH_API_KEY fehlt)' });
      const sellauthService = require('../services/sellauthService');
      const invoice = await sellauthService.getInvoice(apiKey, shopId, invoiceId);
      const products = (invoice.items || []).map(i => ({ product: i.product?.name, variant: i.variant?.name }));
      const checkoutUrl = invoice.unique_id ? `${(shopUrl||'').replace(/\/$/, '')}/checkout/${invoice.unique_id}` : null;
  ...
  /** Zeigt welche Sellauth-Variablen konfiguriert sind (Werte maskiert) */
  async checkSellauthConfig(req, res, next) {
    try {
      ...
      res.json({
        ...
        ready:   !!(apiKey && shopId),
        source: {
          apiKey:  process.env.SELLAUTH_API_KEY  ? 'ENV' : (dbSettings?.sellauth_api_key  ? 'DB' : 'missing'),
          shopId:  process.env.SELLAUTH_SHOP_ID  ? 'ENV' : (dbSettings?.sellauth_shop_id  ? 'DB' : 'missing'),
          shopUrl: process.env.SELLAUTH_SHOP_URL ? 'ENV' : (dbSettings?.sellauth_shop_url ? 'DB' : 'missing'),
        }
      });
    } catch (e) { next(e); }
  },
  ...
  async syncSellauth(req, res, next) {
    try {
      // ENV-Vars haben immer Vorrang — IMMER lesen, nicht nur wenn Body leer ist
      let dbSettings = null;
      try {
        const { data } = await supabase.from('settings')
          .select('sellauth_api_key, sellauth_shop_id, sellauth_shop_url').single();
      ...
      // Nur API Key und Shop ID sind Pflicht — shopUrl ist optional (für Kauflinks)
      const missing = [];
      if (!apiKey) missing.push('SELLAUTH_API_KEY');
